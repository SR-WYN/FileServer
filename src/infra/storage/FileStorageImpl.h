...
public:
    ...
    explicit FileStorageImpl(const std::string &rootDir);
    ~FileStorageImpl() override = default;
    ...
    std::string saveFile(const std::string &category, const std::string &filename, const char *data,
    ...
    /// @param outData      输出缓冲区
    /// @return true 读取成功
    bool readFile(const std::string &relativePath, std::vector<char> &outData) override;
